...
export type OptionSelectItem<T> = {
  value: T;
  label: string;
  description?: string;
  badge?: string;
...
type OptionSelectProps<T> = {
  ...
  options: readonly OptionSelectItem<T>[] | OptionSelectItem<T>[];
  /** Accessible name override for the trigger (defaults to label). */
  triggerAriaLabel?: string;
};

export function OptionSelect<T = string>({
  label,
  value,
  onChange,
  options,
  ...
  return (
    <div ref={rootRef} className="relative w-full">
      ...
      <button
        ...
        className={`group flex w-full items-center justify-between gap-3 rounded-xl border px-4 py-3 text-left transition-all ${open
      ...
      >
        ...
        <span
          className={`shrink-0 text-gray-400 transition-transform duration-200 ${open ? "rotate-180 text-violet-300" : "group-hover:text-gray-300"
            }`}
          aria-hidden
        >
          ...
          {options.map((opt, index) => {
            ...
            return (
              <button
                ...
                className={`flex w-full items-start gap-3 px-3 py-2.5 text-left transition-colors ${isHi
                  ? "bg-violet-500/15"
                  : isSelected
                    ? "bg-white/[0.04]"
                    : "hover:bg-white/[0.06]"
