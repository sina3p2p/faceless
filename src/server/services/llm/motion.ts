...
ONE BEAT ≠ SMALL BEAT (READ THIS): the cure for staticness is INTENSITY within the single beat, not adding beats. A frame that holds only one beat should still feel as alive as the moment demands — a punch-landing frame still feels explosive; a door-slam frame still feels sharp; a glance-up frame can still feel charged. Carry kinetic energy through:
- cameraMove: be aggressive when the beat warrants it — fast push-in, hard arc, whip-pan, snap zoom, rapid handheld. A locked tripod on a single dynamic beat is the #1 cause of "static" feel. Match camera speed to beat intensity, not to beat count.
- subjectDynamics: layer secondary physics RICHLY — hair, cloth, dust, breath, ripples, sparks, smoke, debris, recoil, sway, splatter, whatever the medium and material naturally produce. These are NOT separate beats; they are reactive physics caused by the one primaryAction. Pile them on.
