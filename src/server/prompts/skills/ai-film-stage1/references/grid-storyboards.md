...
4. **The continuity instruction** (the grid's core clause): `"All panels depict the SAME scene, space, characters, and lighting state; consecutive panels are consecutive moments across cuts — each panel begins where the previous panel's action leads; the geography is identical across all panels."`
5. **The Look** folded in as grade/lens character — the grid IS the film's look at storyboard scale.
...
- **Skips (narrow, and only during Step 17)**: a grid skip is valid ONLY for (1) an insert-only scene — every row is INSERT, no geography or edit continuity is being approved — or (2) a lone establishing scene — exactly one shot, no cut handoff depending on scene geography. **A missing grid is NEVER a skip** — and a missing grid TOOL is never grounds for the model to skip on its own: if the environment cannot generate grids, report it and let the USER choose halt vs. grid-less (`environment_no_grid_tooling`, recorded per scene in the registry). A scene with two or more non-insert shots cannot skip; a scene with character blocking, eyelines, screen direction, prop geography, or cut handoffs cannot skip. Every skip is written into the Scene Grid Registry with its `skip_reason`; skipped scenes still get generation groups marked.
