...
3. **Framing matched to the asset's KIND** — the form differs, the purpose (identity capture, not drama) never does. A moody hero-shot makes a beautiful image and a bad reference — the lighting bakes in and fights every shot it anchors:
   - *Characters* → a **sheet**: the prompt MUST invoke the format by name or the image model will draw a cinematic portrait instead. Build it on this skeleton: *"Character reference sheet (turnaround) of [subject + anchor features]: three views of the SAME character — front view, side profile, three-quarter view — standing in a neutral pose, arms relaxed, plain solid light-grey background, flat even studio lighting, no scenery, no props (fixed wardrobe only), full body visible."* Then fold in the Look as palette/grade/film-stock ONLY — never as setting or scene lighting. If a generated candidate has an environment behind the character, scene lighting, or a hand-prop, it is a portrait, not a sheet — regenerate, don't approve. (If your image model renders multi-view sheets poorly — views not matching — fall back to a SET of single-view images on the same neutral background, front/profile/three-quarter as separate generations, all bound to the one handle.)
