...
- **Anchor generation to approved images, never bare text**: character/location references on every shot, and — in this pipeline — the **motion sheet** as the continuous-take trajectory anchor (4–9 panels of one shot; Seedance interpolates between states — no hard cuts; every shot opens with COMPOSITION LOCK on Panel 1 and END STATE LOCK on Panel n — soft "composition matches/follows panel" citations are forbidden; see Stage 2 `shot-compilation-recipe.md`). **Scene continuity packs** (notes + 1–3 keyframes) are required geography references, never Seedance shot sequences or hard-cut boards. Continuity-pack keyframes must never be treated as panels-to-cut.
