...
Load via `loadReference("stage2-skill.md")` only after Stage 1 is complete: Bible locked, every `@material` bound to an approved image, every speaking hero / recurring VO bound to an approved `@*_vo` (or the film is dialogue-free by design), Generation Grid Registry complete and passing. If the registry is missing or any shot lacks both an approved grid handle and a valid skip record, STOP and return to Stage 1 Step 10 — approved assets alone are not sufficient.
