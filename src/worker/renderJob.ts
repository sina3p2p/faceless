...
export async function generateMusicLyricsJob(job: Job<RenderJobData>) {
  ...
  try {
    ...
    const errorMessage = error instanceof Error ? error.message : "Unknown error";
    console.error(`Music lyrics generation failed for ${videoProjectId}:`, errorMessage);
    await updateVideoStatus(videoProjectId, "FAILED");
    await db.update(schema.renderJobs).set({ status: "FAILED", error: errorMessage })
      .where(eq(schema.renderJobs.videoProjectId, videoProjectId));
    throw error;
  }
}
...
export async function generateSongJob(job: Job<RenderJobData>) {
  ...
  try {
    ...
    // Update scenes with actual durations and store alignment/song data in config
    ...
    // Store song metadata in videoProject config
    const updatedConfig = {
      ...videoConfig,
      songUrl: songKey,
    ...
    console.log(`Song generation complete, ready for review`);
  } catch (error) {
    const errorMessage = error instanceof Error ? error.message : "Unknown error";
    console.error(`Song generation failed for ${videoProjectId}:`, errorMessage);
    await updateVideoStatus(videoProjectId, "FAILED");
    await db.update(schema.renderJobs).set({ status: "FAILED", error: errorMessage })
      .where(eq(schema.renderJobs.videoProjectId, videoProjectId));
    throw error;
...
export async function generateMusicVisualsJob(job: Job<RenderJobData>) {
  ...
  try {
    ...
    // Update script with visuals merged in
    if (script) {
    ...
    const errorMessage = error instanceof Error ? error.message : "Unknown error";
    console.error(`Music visuals generation failed for ${videoProjectId}:`, errorMessage);
    await updateVideoStatus(videoProjectId, "FAILED");
    await db.update(schema.renderJobs).set({ status: "FAILED", error: errorMessage })
      .where(eq(schema.renderJobs.videoProjectId, videoProjectId));
    throw error;
...
export async function renderMusicVideoJob(job: Job<RenderJobData>) {
  ...
  try {
    ...
    // Download the pre-generated song from R2
    const songPath = path.join(workDir, "full_song.mp3");
    ...
    // Use Whisper-aligned durations from the song generation step
