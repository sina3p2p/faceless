...
export function VariantNode({ data }: NodeProps) {
  const { url, type, modelUsed, isActive, onClick } = data as VariantNodeData;

  return (
    <>
      <Handle type="target" position={Position.Top} className="w-2! h-2! bg-violet-500/30! border-0!" />
      <button
        onClick={(e) => { e.stopPropagation(); onClick(); }}
        disabled={isActive}
        className={`shrink-0 flex flex-col items-center gap-1 group/vnode transition-all nopan nodrag ${isActive ? "" : "opacity-50 hover:opacity-100"}`}
      >
